...
from classify_images import loadAndCompileModel
...
IMAGEPATH = "./0a5d4570-f987-42eb-bce4-bc484a9d6607_plate.png"
FOLDERPATH =  "./images"
DETECTION_WEIGHT_PATH = "./weights/yolov5_license_plate.pt"
CLASSIFICATION_WEIGHT_PATH = "./weights/weights-improvement-16-1.34.hdf5"
...
def loadDetectionModel(weightPath):
...
def loadClassificationModel(inputShape,numberOfClass,learningRate,weightPath):
...
def predictSeveralImage(folderPath,inputShape,numberOfClass,learningRate,detectionWeighPath,classficationWeighPath):
    
    detectionModel = loadDetectionModel(detectionWeighPath)
    classificationModel = loadAndCompileModel(inputShape,numberOfClass,learningRate,classficationWeighPath)
    
    imagesNameList = myUtils.getImagesNameList(folderPath) 
    imagesPathList = myUtils.getImagesPathList(folderPath)


    predictions = []
    i = 0
    for imagePath in imagesPathList:
        imageName = imagesNameList[i]
        img = Image.open(imagePath)
        try:
            cropImageAsArray = detectionModel(img).crop(save = False)[0]['im']
            augumenImg = agumenImage(cropImageAsArray)
            augumenImg.save(f"./cropImages/{imageName}")
            normalizeImage = normalizeImg(augumenImg)
            prediction = predictImage(normalizeImage,classificationModel)
        except:
...
if __name__ == "__main__":
    predictions = predictSeveralImage(FOLDERPATH,INPUTSHAPE,NUMBEROFCLASSES,LEARNINGRATE,DETECTION_WEIGHT_PATH,CLASSIFICATION_WEIGHT_PATH)
    print(predictions)